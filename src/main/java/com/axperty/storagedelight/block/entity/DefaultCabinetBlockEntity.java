...
public class DefaultCabinetBlockEntity extends LootableContainerBlockEntity {
    ...
    public DefaultCabinetBlockEntity(BlockPos blockPos, BlockState blockState) {
        this(BlockEntityTypesRegistry.DEFAULT_CABINET.get(), blockPos, blockState);
    }

    private DefaultCabinetBlockEntity(BlockEntityType<?> type, BlockPos blockPos, BlockState blockState) {
        ...
        this.viewerManager = new ViewerCountManager() {
            ...
            protected boolean isPlayerViewing(PlayerEntity player) {
                if (player.currentScreenHandler instanceof GenericContainerScreenHandler genericContainerScreenHandler) {
                    ...
                    return inventory == DefaultCabinetBlockEntity.this;
                } else {
                    return false;
                }
            }
    ...
    protected Text getContainerName() {
        return Text.translatable("container.storagedelight.default_cabinet");
    }
    ...
    public void tick() {
        ...
        if (this.viewerManager.getViewerCount() > 0) {
            ...
            if (!(blockstate.getBlock() instanceof DefaultCabinetBlock)) {
                markRemoved();
                return;
            }

            boolean flag = blockstate.get(DefaultCabinetBlock.OPEN);
            if (flag) {
    ...
    private void setOpen(BlockState state, boolean open) {
        Objects.requireNonNull(getWorld()).setBlockState(getPos(), state.with(DefaultCabinetBlock.OPEN, open));
    }

    private void playSound(BlockState state, SoundEvent sound) {
        Vec3i vec3i = state.get(DefaultCabinetBlock.FACING).getVector();
        BlockPos pos = getPos();
